...
internal static class VsMatchGameRules
{
        ...
        VsMatchSession match)
    {
        ...
        MarkAnswerablePlayers(match);
        DeductVitality(match, questioner);
    }
        ...
        DateTime receivedUtc)
    {
        if (match.Phase is not
            ...
            player.HiddenAnswerIndices.Contains(
                request.AnswerIndex) ||
            player.CurrentAnswer is not null ||
            !CanAnswerCurrentQuestion(match, player) ||
            IsLate(match, receivedUtc))
        {
            return false;
        }
    ...
    internal static bool HaveAllParticipantsAnswered(
        ...
        match.Players
            ...
            .Where(player => CanAnswerCurrentQuestion(match, player))
            .All(player =>
                player.CurrentAnswer?.QuestionNumber ==
                match.Game.QuestionNumber);

    internal static bool CanAnswerCurrentQuestion(
        VsMatchSession match,
        VsMatchPlayerState player) =>
        match.Phase != VsMatchPhase.NormalRoundQuestion ||
        player.AnswerableQuestionNumber ==
            match.Game.QuestionNumber;

    internal static void DeductVitality(
        VsMatchSession match,
        VsMatchPlayerState player)
    {
        var character = ResolveCurrentCharacter(match, player);

        if (character.EnergyPoints <= 0)
            return;

        character.EnergyPoints--;

        var rewardTotal = player.CharacterRewardTotals.First(item =>
            item.SlotNumber == character.SlotNumber);
        rewardTotal.EnergyLoss++;
    }
    ...
    private static void ClearAnswers(VsMatchSession match)
    {
        foreach (var player in match.Players)
        {
            player.AnswerableQuestionNumber = 0;
            player.CurrentAnswer = null;
    ...
    private static void MarkAnswerablePlayers(
        VsMatchSession match)
    {
        foreach (var player in match.Players)
        {
            ...
            if (TeamRules.HasVitality(character.EnergyPoints))
            {
                player.AnswerableQuestionNumber =
                    match.Game.QuestionNumber;
            }
    ...
    private static VsMatchCharacterState ResolveCurrentCharacter(
        VsMatchSession match,
        VsMatchPlayerState player)
    {
        var round = ResolveCurrentRound(match, player);

        return player.Characters.First(character =>
            character.SlotNumber == round.CharacterSlotNumber);
    }
