...
public partial class VsMatchPlayView : IDisposable
{
    ...
    private VsHelpCardVm? CurrentHelp =>
        Data.Game.MyHelp is
            { HelpType: VsHelpType.GuessRange } &&
        Data.Phase != VsMatchPhase.NormalRoundGuess
            ? null
            : Data.Game.MyHelp;

    private bool SpeedBonusPoolIncreased =>
        Data.Phase == VsMatchPhase.QuestionResult &&
        Data.Game.QuestionKind == VsQuestionKind.Choice &&
        !Data.Game.QuestionPlayers.Any(player =>
            player.HasSpeedBonus) &&
        Data.Game.QuestionPlayers.Count(player =>
            player.IsCorrect &&
            player.ModifiedTimeSeconds.HasValue) > 1;
