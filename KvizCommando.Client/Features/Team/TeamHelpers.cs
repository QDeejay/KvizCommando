...
{
    internal static class TeamHelpers
    {
        internal static int ResolvePosition(
            bool[] enabledMask,
            int requestedPosition)
        {
            if (requestedPosition >= 1 &&
                requestedPosition <= enabledMask.Length &&
                enabledMask[requestedPosition - 1])
            {
        ...
        internal static string[] RecruitToolTipResolver(bool[] charmask, CandidateDto[] candidate, ILanguageService lang)
        {
            ...
            {
                if (!charmask[i] &&
                    candidate[i] is not null &&
                    !candidate[i].CanBeHire)
                {
