...
public sealed partial class VsMatchService
{
    private void ScheduleBotAnswersLocked(VsMatchSession match)
    {
        ...
        foreach (var bot in match.Players.Where(player =>
                     player.IsBot && player.CurrentAnswer is null))
        {
...
/**
 * ÚJ FÁJL: a VsMatchService bot-időzítését koordinálja. A phase saját
 * cancellation tokenjét használja, a bot a session lockja alatt a
 * tiszta bot-szabályon keresztül válaszol, majd ugyanaz a snapshot-
 * küldés fut le, mint emberi válasznál. Nincs bot SignalR-kapcsolat.
 */
